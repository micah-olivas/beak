...
"""
    ...
    def download(self, job_id: str, local_dir: str = '.') -> Path:
        ...
        DEPRECATED: Use get_results(job_id, parse=False) instead
        """
        import warnings
        warnings.warn(
            "download() is deprecated. Use get_results(job_id, parse=False) instead",
            DeprecationWarning,
            stacklevel=2
        )
        m8_file = self.get_results(job_id, parse=False, download_sequences=False)

        output_dir = Path(local_dir)
        output_dir.mkdir(parents=True, exist_ok=True)
        output_file = output_dir / "results.m8"

        if m8_file.resolve() != output_file.resolve():
            import shutil
            shutil.copy2(m8_file, output_file)
            return output_file
    ...
    def get_hit_sequences(self, job_id: str, output_fasta: Optional[str] = None) -> Path:
        ...
        DEPRECATED: Use get_results(job_id, parse=False, download_sequences=True) instead
        """
        import warnings
        warnings.warn(
            "get_hit_sequences() is deprecated. Use get_results(job_id, parse=False, download_sequences=True) instead",
            DeprecationWarning,
            stacklevel=2
        )

        result = self.get_results(job_id, parse=False, download_sequences=True)
